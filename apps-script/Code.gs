...
function doPost(e) {
  try {
    ...
    const user = verifyUser_(e.parameter.idToken);
    const payload = JSON.parse(e.parameter.payload || '{}');
    if (action === 'session') return json_({ok: true, user});
...
function verifyUser_(idToken) {
  if (!idToken) throw new Error('請先使用 Google 登入。');
  ...
  const match = rows.slice(1).find((row) => row[2].toLowerCase() === identity.email.toLowerCase() && row[4] === 'Active');
